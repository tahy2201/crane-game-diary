...
```
src/components/
├── shadcn-ui/   # shadcn/ui が自動生成するコンポーネント（lowercase ファイル名）
│                # 例: button.tsx, input.tsx, badge.tsx
│                # `npx shadcn@latest add` で追加する。直接編集しても OK
│
├── layout/      # 画面レイアウトに関わる共通コンポーネント
│                # 例: BottomNavigation.tsx, FloatingActionButton.tsx, Modal.tsx
│
├── arcade/      # ゲーセン機能に関連するコンポーネント（今後追加予定）
├── play/        # プレイ記録機能に関連するコンポーネント（今後追加予定）
├── prize/       # 景品機能に関連するコンポーネント（今後追加予定）
│
├── AuthGuard.tsx  # 全画面横断で使う共通コンポーネントはルート直下に置く
└── ...
```

### ファイル命名規則

- `src/components/shadcn-ui/` 以外のコンポーネントはすべて **PascalCase** で命名する
  - 例: `BottomNav.tsx`, `FloatingActionButton.tsx`, `AuthGuard.tsx`
- 略称・略語は使わない（例: `FAB` → `FloatingActionButton`、`BottomNav` → `BottomNavigation`）
- `src/components/shadcn-ui/` は shadcn/ui の自動生成に合わせて lowercase のまま維持する
- `index.tsx` / `main.tsx` はバンドラーが解決するエントリーポイントのため lowercase が正しい（例: `src/pages/prize/index.tsx` は `import '@/pages/prize'` で解決される）
...
| パス | ページ | 備考 |
|------|--------|------|
| `/` | タイムライン（リダイレクト） | `/timeline` へ転送 |
| `/timeline` | タイムライン | |
| `/arcades` | ゲーセン一覧 | |
| `/prizes` | 景品一覧 | |
| `/settings` | 設定 | |
| `/group/new` | グループ作成（S-02） | グループ未所属の場合などに表示 |
| `/master` | マスタ管理（S-04） | 景品カテゴリ・クレーン形式の管理。編集モーダル（S-05・S-06）はこの画面から開く |
| `/owner-transfer` | owner移譲（S-07） | owner がアカウント削除する際に他メンバーがいる場合に表示 |
| `/delete-account` | アカウント削除確認（S-08） | パスワード再入力で本人確認後に論理削除 |
| `/login` | ログイン | |
| `/signup` | サインアップ | |
| `*` | 404 | |
...
- 例外として `src/components/shadcn-ui/` は `a11y/noLabelWithoutControl` を無効化している
